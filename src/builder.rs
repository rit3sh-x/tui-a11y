...
///
/// Terminal UIs generally have no retained widget tree to derive an
/// accessibility tree from (see the crate docs), so the app declares one
/// explicitly, alongside its normal rendering code. Build bottom-up: create
/// child nodes first so you have their ids on hand when you create the
/// parent.
///
/// ```
/// use tui_a11y::{Role, TreeBuilder, node_id};
///
...
impl TreeBuilder {
    ...
    pub fn build(self) -> TreeUpdate {
        ...
        let root = self.root.unwrap_or_else(|| {
            let id = node_id("tui-a11y:missing-root");
